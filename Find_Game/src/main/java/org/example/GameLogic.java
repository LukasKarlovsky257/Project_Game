...
public class GameLogic {
    ...
    private Scanner scan = new Scanner("level.txt");
    ...
    public GameLogic() {
        ...
        this.level = 1;
        try {
            this.writer = new FileWriter("level.txt");
            writer.write("1");
            writer.close();
        } catch (IOException e) {
    ...
    public void update() throws IOException {
        for (Wall wall: walls) {
            if (scan.nextLine() == "1") {
                setLevel(1);
            } else if (item1.isCollided(wall.getRectangle().getBounds())) {
    ...
    public void collision() {
            if (getLevel() == 1) {
                if (player.isCollided(bushes.get(2).getRectangle())) {
            ...
            if (getLevel() == 2) {
                if (player.isCollided(bushes.get(6).getRectangle())) {
                    System.out.println("Kolize");
                    item1.move(ENEMY_STEPS, UP);
                } else {
                    System.out.println("Nic");
            ...
            if (getLevel() == 3) {
                if (player.isCollided(bushes.get(8).getRectangle())) {
                    System.out.println("Kolize");
                    item1.move(ENEMY_STEPS, UP);
                } else {
                    System.out.println("Nic");
